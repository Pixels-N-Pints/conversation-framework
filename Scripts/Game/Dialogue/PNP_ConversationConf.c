...
class PNP_ConversationConf
{
	[Attribute(desc: "Dialogue tree")]
	protected ref array<ref PNP_DialogueEntryConf> options;
	
	ref array<ref PNP_DialogueEntryConf> GetOptions()
	{
		return options;
	}
}
...
[BaseContainerProps(configRoot: true)]
class PNP_DialogueEntryOptionConf
{
	[Attribute(desc: "Message displayed above the options if any. You can interpolate expressions (eg. {IDENTITY_NAME}) to include the identity of the player or npc in the message. See PNP_StringTemplating for more details.")]
...
class PNP_DialogueEntryConf : PNP_DialogueEntryOptionConf
{
	...
	ref array<ref PNP_DialogueEntryOptionConf> GetOptions()
	{
		return m_Options;
	}
